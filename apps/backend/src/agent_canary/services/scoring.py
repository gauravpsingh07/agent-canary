...
def score_groundedness(
    ...
    if normalized_category(payload) == "hallucination" and answer_contains_unsupported_claim(
        output,
        payload.retrieved_evidence,
    ):
        failure_reasons.append("Agent answer contained a claim unsupported by retrieved evidence.")
        return 0
...
def requires_citations(payload: EvaluationScoringInput) -> bool:
    return (
        normalized_category(payload) == "weak_retrieval"
...
def answer_contains_unsupported_claim(
    ...
    if "guarantee" not in answer:
        return False
    evidence_text = " ".join(
        str(evidence.get("content") or "").lower()
        for evidence in retrieved_evidence
    )
    return "guarantee" not in evidence_text
